...
constructor(private val userService: DbUserService,
  ...
  override fun createBook(@RequestBody book: Book): Book {
    if (book.id != null) {
      throw BadRequest("Cannot create book with existing id=$book.id")
    }
  ...
  override fun setBook(@PathVariable("bookId") id: Long, @RequestBody book: Book): Book {
  ...
  private class BookWithOwner(private val book: Book, override val owner: User) : Book {
