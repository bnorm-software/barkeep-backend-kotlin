...
public final class RecipesView extends VerticalLayout implements View {
  ...
  public void detach() {
    super.detach();
    // A new instance of RecipesView is created every time it's
      ...
      public void textChange(final TextChangeEvent event) {
        ...
        data.addContainerFilter(new Filter() {
          ...
          public boolean passesFilter(final Object itemId, final Item item) {
            ...
            return filterByProperty("title", item, text) || filterByProperty("owner", item, text);

          }

          @Override
          public boolean appliesToProperty(final Object propertyId) {
            return propertyId.equals("title") || propertyId.equals("owner");
          }
        });
      }
    });
  ...
  private Table buildTable() {
    ...
    BeanContainer<Long, RecipeBean> newDataSource = new BeanContainer<>(RecipeBean.class);
    newDataSource.setBeanIdProperty("id");
    try {
      BarkeepService service = VaadinSession.getCurrent().getAttribute(BarkeepService.class);
      List<Recipe> recipes = service.getRecipes().execute().body();
      for (Recipe recipe : recipes) {
        ...
        Set<com.bnorm.barkeep.model.Component> components = recipe.getComponents();
        if (components != null) {
          bean.setComponents(components.stream().map(c -> {
            StringBuilder builder = new StringBuilder();
            builder.append(c.getMin());
            if (c.getMax() != null) {
              builder.append(" to ").append(c.getMax());
            }
            builder.append(" of ");
            Ingredient ingredient = c.getIngredient();
            assert ingredient != null;
            builder.append(ingredient.getTitle());
    ...
    table.setVisibleColumns("title", "owner", "components");
    table.setColumnHeaders("Title", "Owner", "Components");
    ...
    table.addActionHandler(new RecipesActionHandler());
  ...
  void createNewReportFromSelection() {
    ...
    //    DashboardEventBus.post(new DashboardEvent.RecipeReportEvent((Collection<Recipe>) table.getValue()));
  }
  ...
  private class RecipesActionHandler implements Handler {
